...
class SiabArticulosController extends Controller
{
    ...
    public function GetProductosByBodDesS(Request $request){
        try {
            $get = siab_articulos::select('siab_articulo.id','siab_articulo.CODART_TRACK','siab_articulo.NOMBRE','siab_articulo.GENERICO',
            'siab_articulo.CAT_FARMACIA','siab_articulo.UNIMEDBASE','siab_articulo.CODART_ONU','siab_articulo.CONCENTRACION',
            'siab_articulo.CODART','siab_articulo.CODART_BARR','auth_estados.descripcionEstado',
           ...
           ->leftjoin('bodega','siab_articulo.idBodega','=','bodega.id')
           ->leftjoin('zona','siab_articulo.idZona','=','zona.id')
           ->where('siab_articulo.idBodega',$request->idBodega)
           ->where("siab_articulo.CODART",'like', '%'.$request->DATA.'%')
           ->orwhere("siab_articulo.NOMBRE",'like', '%'.$request->DATA.'%')
           //->orwhere("siab_articulo.CODART_BARR",'like', '%'.$request->DATA.'%')
           ->get();
            return $get;
        } catch (\Throwable $th) {
            log::info($th);
    ...
    public function GetProductosByBodDes(Request $request){
        try {
           ...
           WHERE siab_articulo.idBodega = ".$request->idBodega." && siab_articulo.NOMBRE LIKE '%".$request->DATA."%' || siab_articulo.CODART_BARR LIKE '%".$request->DATA."%' || 
           siab_articulo.CODART LIKE '%".$request->DATA."%'");
